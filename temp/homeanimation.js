...
$(window).on("load", function () {
	...
	function stopShow(typeOfBubble, buttonName) {
		...
		if (changedButtonText == 'Everything' && $(buttonName).text() != 'Everything') {
			showAll.addEventListener('click', function () {
				stopShow($bubbles, showAll);
				buttonName.removeEventListener('click', arguments.callee);
			}, false);
		}
	...
	function bubbleShow() {
		...
		function floatUp() {
			...
			$($bubbles[bubbleIndex]).animate({ left: bubbleOrigin + 'px' }, 0).show(0).animate({ left: bubbleOrigin + 'px' }, 0).animate({ opacity: 100 }, 0).animate({ left: '+=' + horizontalA + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear').animate({ left: '+=' + horizontalB + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear').animate({ left: '+=' + horizontalA + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear').animate({ left: '+=' + horizontalB + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear').animate({ left: '+=' + horizontalA + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear' /*,function(){indexUpdate(),floatUp()}*/).animate({ left: '+=' + horizontalB + 'px', bottom: '+=' + (containerHeight / 6 - bubbleHeight / 6) + 'px' }, 1000 * speed, 'linear').animate({ opacity: 0 }, 700).animate({ bottom: '0' }, 0, 'linear').hide(0);
			indexUpdate();
